...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES["acceptance_signature_img"]) && $_FILES["acceptance_signature_img"]["error"] == 0) {
        $filename = time() . '-' . basename($_FILES["acceptance_signature_img"]["name"]);
        $target_file = $target_dir . $filename;
        if (move_uploaded_file($_FILES["acceptance_signature_img"]["tmp_name"], $target_file)) {
            $file_paths['acceptance_signature_img'] = $filename;
        }
    }
