...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES["internship_certificate_file"]) && $_FILES["internship_certificate_file"]["error"] == 0) {
        // กำหนดโฟลเดอร์ที่จะบันทึกไฟล์
        $target_dir = "../uploads/";
        // แสดงนามสกุลของไฟล์ที่อัปโหลด
        $file_extension = strtolower(pathinfo($_FILES["internship_certificate_file"]["name"], PATHINFO_EXTENSION));
        ...
        if (in_array($file_extension, ['pdf', 'png', 'jpg', 'jpeg'])) {
            ...
            $filename = time() . '-' . basename($_FILES["internship_certificate_file"]["name"]);
            // เตรียมพาธของไฟล์ที่จะบันทึก
            $target_file = $target_dir . $filename;
            // ย้ายไฟล์ไปยังโฟลเดอร์ที่กำหนด
            if (move_uploaded_file($_FILES["internship_certificate_file"]["tmp_name"], $target_file)) {
                // ทำการอัปเดตข้อมูลในฐานข้อมูล
                $sql = "UPDATE internship_certificate SET 
                            internship_certificate_file = '" . mysqli_real_escape_string($conn, $filename) . "',
                            status_admin = 1
                        WHERE email = '$email'";
                // ทำการอัปเดตข้อมูล
                if (mysqli_query($conn, $sql)) {
                    ...
                    echo "}).then(function () {";
                    echo "    window.location ='./certificate_detail.php';";
                    echo "});";
                    echo "</script>";
                } else {
