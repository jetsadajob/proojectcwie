    ...
    if ($result) { echo '<script>
        setTimeout(function() {
            swal({
                ...
                window.location = "./submitdocinternship_detail.php?id=' . $email . '"; // ส่งกลับไปยังหน้า detilcoop.php พร้อมกับ id
            });
        }, 1000);
    </script>';


        // header("Location: submitdocinternship_detail.php?id=$email"); // ส่งกลับไปยังหน้า detilinterns.php พร้อมกับ id
        exit;
    } else {
        echo '<script>
        setTimeout(function() {
